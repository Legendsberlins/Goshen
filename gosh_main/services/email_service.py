...
def send_contact_email(name, email, subject, message):
    ...
    # Try SendGrid Web API first (bypasses SMTP firewall issues)
    try:
        ...
        if api_key:
            ...
            logger.info(f"Contact email sent via Web API, status: {response.status_code}")
            return True
    except ImportError:
        logger.warning("sendgrid package not installed, falling back to SMTP")
    except Exception as e:
        logger.error(f"SendGrid Web API failed: {e}, falling back to SMTP")
...
def send_password_reset_email(user_email, message_body, subject="Reset your Goshen password"):
    ...
    try:
        ...
        if api_key:
            ...
            sg = SendGridAPIClient(api_key)
            response = sg.send(mail)
            logger.info(f"Password reset email sent via Web API, status: {response.status_code}")
            return True
    except ImportError:
        logger.warning("sendgrid package not installed, falling back to SMTP")
    except Exception as e:
